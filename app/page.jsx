...
const Page = () => {
  ...
  useEffect(()=>{
    ...
    if(!hasRunOnce && session?.user){
      ...
      snackBar.open("success",{ ...snackBarSucessdata, 
        message: <div className="flex gap-1 w-full">Login Successfull <div className="max-w-16 lg:max-w-24 truncate ... font-bold">{`${session?.user.name}`}</div></div>
      }, 7000);
    }
  }, [session])
